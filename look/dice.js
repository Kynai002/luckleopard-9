...
function denyWager(message){verdictView.textContent=message;wagerInput.classList.remove('rl-deny-30528957');void wagerInput.offsetWidth;wagerInput.classList.add('rl-deny-30528957');}
...
rolling=true;rollBtn.disabled=true;clearFlash();wagerInput.classList.remove('rl-deny-30528957');purse=round2(purse-wager);savePurse();renderPurse();var target=currentTarget();var prizeRate=payoutFor(target);verdictView.textContent='The die is rolling…';dieView.classList.add('rl-spin-30528957');scrambleTimer=window.setInterval(function(){dieView.textContent=String(Math.floor(Math.random()*100)+1);},SCRAMBLE_TICK_MS);window.setTimeout(function(){settleRoll(wager,target,prizeRate);},SCRAMBLE_MS);}
...
var brandEl=document.querySelector('.rl-brand-30528957');var echoEl=grab('rl-echo');if(brandEl&&echoEl){echoEl.textContent=brandEl.textContent.replace(/\s+/g,' ').trim();}
